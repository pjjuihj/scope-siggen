...
/* USER CODE BEGIN PV */
/* 外部任务句柄声明 */
extern osThreadId_t oscilloscope_task_handle;
extern osThreadId_t signalgen_task_handle;
extern osThreadId_t uart_task_handle;
...
int main(void)
{
  ...
  if (HAL_I2C_Master_Transmit(&hi2c1, 0x78, &test_byte, 1, 100) != HAL_OK) {
    char warn[] = "WARN: I2C OLED not responding\r\n";
    HAL_UART_Transmit(&huart1, (uint8_t*)warn, sizeof(warn)-1, 100);
  }
  ...
  /* 应用初始化 */
  BootStage_t boot_stage = App_Init();

  if (boot_stage == BOOT_STAGE_ERROR) {
...
static void MX_TIM5_Init(void)
{
  ...
  htim5.Instance = TIM5;
  htim5.Init.Prescaler = 0;  /* 84MHz / (0+1) / 256 = 328kHz TRGO, 运行时 SigGen_ApplyConfig 会重新计算 */
  htim5.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim5.Init.Period = 256-1;  /* DAC_BUFFER_SIZE - 1 */
...
static void MX_TIM8_Init(void)
{
  ...
  htim8.Init.Period = 16799;  /* 84MHz / 16800 = 5kHz, 运行时 Osc_ApplyConfig 会重新计算 */
