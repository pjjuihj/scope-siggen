...
**做法**: 一次性写了 400+ 行代码，4 个模块同时推进。

**结果**: 编译通过，但没有任何功能经过硬件验证。OLED 乱码、DAC 不输出、ADC 不采集。

**教训**: 写代码 ≠ 跑通功能。先用最丑的方式把一条链路跑通，再美化。

### Day 2（06-27）：3 小时修 8 个 bug

**做法**: 逐个外设验证，发现 8 个初始化配置 bug。
...
**关键发现**: 所有 bug 都是初始化问题。如果第一天做了外设自检，这 8 个 bug 会在 10 分钟内全部发现：
...
**教训**: 寄存器不会骗人。软件层面猜来猜去，不如直接读硬件状态。

### Day 2 下午：配置系统连接 + ADC 配置修复

**做法**: 连接 Config 系统与 ADC/DAC 硬件，修复 ADC 硬件触发配置。

**关键修复**:
- ADC 外部触发模式的 4 个必要配置
- ADC 采样时间从 84 cycles 改为 480 cycles（高阻信号源必须）
- 启动顺序：先 ADC DMA，后 TIM8

**方法论转变**: 试错 2 小时 → 查 GitHub 5 分钟 → 一次修复。

### Day 2 晚：功能扩展 + 代码审查

**做法**: 显示优化、触发系统、信号发生器优化。
...
**做法**: 审查 DMA 缓冲区数据流，修复竞态条件。

**关键修复**:
- process_ptr/process_len 竞态窗口
- HAL_TIM_Base_Stop_IT 用错
- Oscilloscope_Task 栈从 2KB 增大到 4KB
...
**额外**: 启用 IWDG 看门狗（32秒超时）+ 16 条自动化回归测试。

---

## 四、关键调试经验

### 4.1 ADC 外部触发配置（4 个必要配置）
...
**原因**: RM0090 Section 11.3.15 要求 ADC 先就绪，定时器后启动。

### 4.3 ADC 采样时间必须匹配信号源阻抗
...
**踩坑**: DAC 输出缓冲器阻抗 ~15kΩ，用 84 cycles 采样值只有 60% 正确。
...
**踩坑**: 忘记考虑 ARR，导致 DAC 频率偏低 256 倍。

### 4.5 波形显示必须用 min/max 包络
...
**验证永远是你的事。** 烧录、测量、读寄存器——这些 AI 做不了，你必须自己做。
